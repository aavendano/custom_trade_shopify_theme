...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function Ajax(Alpine2) {
    ...
    Alpine2.directive("target", (el, { value, modifiers, expression }, { evaluateLater: evaluateLater2, effect: effect3 }) => {
      let setTarget = (ids) => {
    ...
    Alpine2.directive("headers", (el, { expression }, { evaluateLater: evaluateLater2, effect: effect3 }) => {
      let evaluate2 = evaluateLater2(expression || "{}");
      effect3(() => {
        evaluate2((headers) => {
    ...
    Alpine2.directive("merge", (el, { value, modifiers, expression }, { evaluateLater: evaluateLater2, effect: effect3 }) => {
  ...
  async function send(control, action = "", method = "GET", body = null, enctype = "application/x-www-form-urlencoded") {
    ...
    let renders = PendingTargets.get(response).map(async (target) => {
      ...
      if (!dispatch2(target, "ajax:merge", { strategy, content, merge: render2 })) {
        return;
      }
      return render2();
    });
